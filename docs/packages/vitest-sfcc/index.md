...
Run `sfcc-ts-sync-types` from `@commerce-klaus/typescript-sfcc` to generate the project-specific `SfccJobSteps.Definitions` registry. When it is present, `loadSfccJobStep()` completes and validates registered type IDs, and `run()` uses the parameters, required fields, defaults, and enum values declared by that step. Projects without the generated registry retain the general string and parameter-record API.
...
Before invoking the module, `run()` applies declared defaults and `@trim`, then normalizes Boolean, numeric, string, date/time, and `@target-type` values. It also validates declared `enum-values`. This covers SFCC metadata that stores Boolean, numeric, and date defaults as strings. Missing required parameters and invalid converted values reject with a diagnostic containing the type ID and parameter name. Parameters not declared in `steptypes.json` pass through unchanged for test-specific inputs.
