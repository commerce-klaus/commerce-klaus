...
export function generateCustomAttributesTypes(
  ...
  const typesRootDirectory = path.join(options.workspaceRoot, ".b2c-script-types", "types")
  const metaDirectory = path.join(
    resolveSiteTemplatePath(options.workspaceRoot, options.siteTemplatePath),
    "meta",
  )
  const dwTypesDirectory = path.join(typesRootDirectory, "dw")
...
function detectDwGlobalNamespace(
  typesRootDirectory: string,
  existsSync: (filePath: string) => boolean,
  readFileSync: (filePath: string, encoding: BufferEncoding) => string,
): boolean {
  const globalTypesPath = path.join(typesRootDirectory, "global.d.ts")
...
function renderGeneratedDeclarations(
  ...
  if (hasDwGlobalNamespace) {
    ...
    if (requireOverloads.length > 0) {
      ...
      for (const overload of requireOverloads) {
        lines.push(`  function require${overload.slice(0)}`)
      }
      lines.push("}")
      lines.push("")
...
function renderDwRequireOverloads(dwModuleIndex: Map<string, string[]>): string[] {
  const moduleSpecifiers = new Set<string>()
  for (const candidates of dwModuleIndex.values()) {
    for (const moduleSpecifier of candidates) {
      if (moduleSpecifier.startsWith("dw/")) {
  ...
  return [...moduleSpecifiers]
    ...
    .map((moduleSpecifier) => {
      const globalType = toGlobalDwTypeExpression(moduleSpecifier)
      if (!globalType) {
        return undefined
      }

      return `(id: "${moduleSpecifier}"): ${globalType}`
    })
    .filter((line): line is string => Boolean(line))
}

function toGlobalDwTypeExpression(moduleSpecifier: string): string | undefined {
  const segments = moduleSpecifier.split("/")
  if (segments[0] !== "dw" || segments.length < 2) {
    return undefined
  }

  const identifierPattern = /^[A-Za-z_$][A-Za-z0-9_$]*$/u
  if (!segments.slice(1).every((segment) => identifierPattern.test(segment))) {
    return undefined
  }
