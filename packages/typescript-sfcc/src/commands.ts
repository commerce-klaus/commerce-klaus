...
import { renderCleanGeneratedTypes } from "./clean-types-output.ts"
import { cleanGeneratedTypes, type CleanGeneratedTypesResult } from "./clean-types.ts"
...
export class CleanTypesCommand extends CommerceKlausCommand {
  static description = "Remove project-specific SFCC types generated by Commerce Klaus"
  static enableJsonFlag = true
  static examples = ["<%= config.bin %>", "<%= config.bin %> --dry-run"]
  static usage: string | string[] | undefined = "[flags]"
  static flags = {
    help: Flags.boolean({ char: "h", description: "Show CLI help." }),
    "dry-run": Flags.boolean({
      description: "List generated type files without removing them",
    }),
  ...
  async run(): Promise<CleanGeneratedTypesResult> {
    const { flags } = await this.parse(CleanTypesCommand)
    if (flags.help) {
    ...
    let result: CleanGeneratedTypesResult
    try {
      result = cleanGeneratedTypes({
        currentDirectory,
    ...
    if (!this.jsonEnabled()) {
      ux.stdout(renderCleanGeneratedTypes(result, ux.colorize))
    }
